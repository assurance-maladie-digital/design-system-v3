<script setup lang="ts">
	...
	const headers = ref([
		...
		{ title: 'Email', value: 'email' }, // supported in your table layer
	])
	...
	// Now receives real objects (because rows have no id)
...
<template>
	...
	>
		...
		<div
			...
			:key="(item as any).email || index"
			class="mb-2 pa-2 bg-grey-lighten-3"
		>
			<div><strong>Nom:</strong> {{ (item as any).lastname }}</div>
			<div><strong>Prénom:</strong> {{ (item as any).firstname }}</div>
			<div><strong>Email:</strong> {{ (item as any).email }}</div>
		</div>
	</div>
</template>
