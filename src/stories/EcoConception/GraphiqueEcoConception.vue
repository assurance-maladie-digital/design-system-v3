<script setup lang="ts">
	...
	type DomainData = {
		...
		level1Status: string
		level2Status: string
		level1Action: DomainAction
		level2Action: DomainAction
	}
	...
	const domainData: Record<Domain, DomainData> = {
		strategy: {
			label: 'Stratégie',
			...
			level1Value: 'wip',
			level2Value: '9',
			level1Status: 'Work in Progress',
			level2Status: '9 bonnes pratiques disponibles',
			level1Action: {
				label: 'Consulter les travaux du niveau 1',
				link: '/?path=/docs/éco-conception-bonnes-pratiques-essentielles--docs',
			},

			level2Action: {
				label: 'Consulter le référentiel stratégie',
			},
		},
		architecture: {
			...
			level2Value: '21',
			level1Status: 'Work in Progress',
			level2Status: '21 bonnes pratiques disponibles',
			level1Action: {
				label: 'Consulter les travaux du niveau 1',
				link: '/?path=/docs/éco-conception-bonnes-pratiques-essentielles--docs',
			},
			level2Action: {
				label: 'Consulter le référentiel interne',
			},
		},

		ux: {
			label: 'UX / UI / Développement Front',
			shortLabel: 'UX / UI / Dev Front',
			color: '#EC4899',
			level1Value: '20',
			level2Value: '43',
			level1Status: '20 bonnes pratiques disponibles',
			level2Status: '43 bonnes pratiques disponibles',
			level1Action: {
				label: 'Consulter les pratiques du niveau 1',
				link: '/?path=/docs/éco-conception-bonnes-pratiques-essentielles--docs',
			},
			level2Action: {
				label: 'Demander le référentiel interne',
			},
		},
		...
		backend: {
			...
			level2Value: '12',
			level1Status: 'Work in Progress',
			level2Status: '12 bonnes pratiques disponibles',
			...
			level2Action: {
				label: 'Consulter le référentiel back',
			},
		},
		...
		maintenance: {
			...
			level2Value: '18',
			level1Status: 'Work in Progress',
			level2Status: '18 bonnes pratiques disponibles',
			level1Action: {
				label: 'Consulter les travaux du niveau 1',
				link: '/?path=/docs/éco-conception-bonnes-pratiques-essentielles--docs',
			},
			level2Action: {
				label: 'Consulter le référentiel maintenance',
			},
		},
	}
	...
	const selectedData = computed(() => {
		...
		return domainData[selectedSelection.value.domain]
	})

	const selectedValue = computed(() => {
		if (!selectedSelection.value || !selectedData.value)
			return ''

		return selectedSelection.value.level === 1
			? selectedData.value.level1Value
			: selectedData.value.level2Value
	})
	...
	const selectedStatus = computed(() => {
		...
		return selectedSelection.value.level === 1
			? selectedData.value.level1Status
			: selectedData.value.level2Status
	})
	...
	const selectedAction = computed(() => {
		...
		return selectedSelection.value.level === 1
			? selectedData.value.level1Action
			: selectedData.value.level2Action
	})
	...
	>
		...
		>
			...
			<g
				...
				@mouseenter="activate('strategy', 1)"
				@mouseleave="deactivate"
				@focus="activate('strategy', 1)"
				@blur="deactivate"
				@click="selectDomain('strategy', 1)"
				@keydown.enter.prevent="selectDomain('strategy', 1)"
				@keydown.space.prevent="selectDomain('strategy', 1)"
			>
				<path
					d="M673 425C708.325 425 742.966 434.746 773.11 453.165L731.397 521.429C713.814 510.685 693.607 505 673 505L673 425Z"
					fill="#D1BEFB"
				/>
			...
			<g
				...
				@mouseenter="activate('architecture', 1)"
				@mouseleave="deactivate"
				@focus="activate('architecture', 1)"
				@blur="deactivate"
				@click="selectDomain('architecture', 1)"
				@keydown.enter.prevent="selectDomain('architecture', 1)"
				@keydown.space.prevent="selectDomain('architecture', 1)"
			>
				<path
					d="M777.979 456.241C811.652 478.23 837.57 510.238 852.076 547.747C866.581 585.257 868.943 626.374 858.826 665.298L781.399 645.174C787.3 622.468 785.922 598.483 777.461 576.603C768.999 554.722 753.88 536.051 734.238 523.224L777.979 456.241Z"
					fill="#F7E19C"
				/>
			...
			<g
				...
				@mouseenter="activate('ux', 1)"
				@mouseleave="deactivate"
				@focus="activate('ux', 1)"
				@blur="deactivate"
				@click="selectDomain('ux', 1)"
				@keydown.enter.prevent="selectDomain('ux', 1)"
				@keydown.space.prevent="selectDomain('ux', 1)"
			>
				<path
					d="M857.294 670.85C845.785 710.238 821.96 744.902 789.311 769.76C756.663 794.618 716.91 808.363 675.88 808.978C634.85 809.594 594.702 797.048 561.323 773.18C527.944 749.312 503.09 715.378 490.404 676.354L566.486 651.623C573.886 674.387 588.384 694.182 607.855 708.105C627.326 722.028 650.746 729.346 674.68 728.987C698.614 728.628 721.804 720.611 740.848 706.11C759.893 691.609 773.791 671.389 780.505 648.412L857.294 670.85Z"
					fill="#F7B6D6"
				/>
			...
			<g
				...
				@mouseenter="activate('backend', 1)"
				@mouseleave="deactivate"
				@focus="activate('backend', 1)"
				@blur="deactivate"
				@click="selectDomain('backend', 1)"
				@keydown.enter.prevent="selectDomain('backend', 1)"
				@keydown.space.prevent="selectDomain('backend', 1)"
			>
				<path
					d="M488.706 670.85C475.122 624.359 479.61 574.452 501.272 531.132L572.825 566.91C560.189 592.181 557.571 621.293 565.495 648.412L488.706 670.85Z"
					fill="#BDE6F7"
				/>
			...
			<g
				...
				@mouseenter="activate('maintenance', 1)"
				@mouseleave="deactivate"
				@focus="activate('maintenance', 1)"
				@blur="deactivate"
				@click="selectDomain('maintenance', 1)"
				@keydown.enter.prevent="selectDomain('maintenance', 1)"
				@keydown.space.prevent="selectDomain('maintenance', 1)"
			>
				<path
					d="M503.925 526.019C519.894 496.342 543.393 471.391 572.061 453.674C600.729 435.957 633.555 426.097 667.241 425.086L669.64 505.05C649.991 505.64 630.842 511.391 614.119 521.727C597.396 532.062 583.688 546.616 574.373 563.928L503.925 526.019Z"
					fill="#E7B3AB"
				/>
			...
			<g
				...
				@mouseenter="activate('strategy', 2)"
				@mouseleave="deactivate"
				@focus="activate('strategy', 2)"
				@blur="deactivate"
				@click="selectDomain('strategy', 2)"
				@keydown.enter.prevent="selectDomain('strategy', 2)"
				@keydown.space.prevent="selectDomain('strategy', 2)"
			>
				<path
					d="M673 247C741.075 247 807.831 265.781 865.92 301.275L803.351 403.673C764.102 379.69 718.997 367 673 367L673 247Z"
					fill="#8B5CF6"
				/>
			...
			<g
				...
				@mouseenter="activate('architecture', 2)"
				@mouseleave="deactivate"
				@focus="activate('architecture', 2)"
				@blur="deactivate"
				@click="selectDomain('architecture', 2)"
				@keydown.enter.prevent="selectDomain('architecture', 2)"
				@keydown.space.prevent="selectDomain('architecture', 2)"
			>
				<path
					d="M875.303 307.204C940.194 349.579 990.14 411.26 1018.09 483.544C1046.05 555.828 1050.6 635.065 1031.1 710.074L914.961 679.888C928.134 629.206 925.059 575.668 906.171 526.827C887.284 477.987 853.536 436.31 809.691 407.679L875.303 307.204Z"
					fill="#EAB308"
				/>
			...
			<g
				...
				@mouseenter="activate('backend', 2)"
				@mouseleave="deactivate"
				@focus="activate('backend', 2)"
				@blur="deactivate"
				@click="selectDomain('backend', 2)"
				@keydown.enter.prevent="selectDomain('backend', 2)"
				@keydown.space.prevent="selectDomain('backend', 2)"
			>
				<path
					d="M317.851 720.773C291.672 631.182 300.322 535.007 342.065 451.524L449.395 505.192C421.19 561.599 415.346 626.583 433.034 687.117L317.851 720.773Z"
					fill="#5BC0EB"
				/>
			...
			<g
				...
				@mouseenter="activate('maintenance', 2)"
				@mouseleave="deactivate"
				@focus="activate('maintenance', 2)"
				@blur="deactivate"
				@click="selectDomain('maintenance', 2)"
				@keydown.enter.prevent="selectDomain('maintenance', 2)"
				@keydown.space.prevent="selectDomain('maintenance', 2)"
			>
				<path
					d="M347.178 441.672C377.952 384.482 423.237 336.4 478.483 302.257C533.728 268.115 596.987 249.115 661.902 247.166L665.501 367.112C621.64 368.429 578.897 381.267 541.569 404.336C504.241 427.405 473.643 459.894 452.85 498.535L347.178 441.672Z"
					fill="#C2412D"
				/>
			...
			>
				...
				<!-- <text
					x="673"
					y="407"
					class="schema-level-text__description schema-level-text__description--level-2"
					text-anchor="middle"
					>
					103 bonnes pratiques avancées
					</text> -->
			</g>
			...
			>
				...
				>
					{{ domainData.strategy.level1Value }}
				</text>
				...
				<text
					...
					fill="#8B5CF6"
				>
					{{ domainData.strategy.level2Value }}
				</text>
			</g>
			...
			>
				...
				<text
					...
					class="schema-score__text schema-score__text--light"
				>
					{{ domainData.architecture.level1Value }}
				</text>
				...
				<text
					...
					fill="#EAB308"
				>
					{{ domainData.architecture.level2Value }}
				</text>
			</g>
			...
			>
				...
				<text
					...
					class="schema-score__text schema-score__text--light"
				>
					{{ domainData.backend.level1Value }}
				</text>
				...
				<text
					...
					fill="#5BC0EB"
				>
					{{ domainData.backend.level2Value }}
				</text>
			</g>
			...
			>
				...
				<text
					...
					class="schema-score__text schema-score__text--light"
				>
					{{ domainData.maintenance.level1Value }}
				</text>
				...
				<text
					...
					fill="#C2412D"
				>
					{{ domainData.maintenance.level2Value }}
				</text>
			</g>
			...
			>
				<path
					d="M774.389 254.131L787.12 208.888"
				...
				<text
					...
					text-anchor="middle"
				>
					{{ domainData.strategy.shortLabel }}
				</text>

				<path
					d="M1026.56 480.662L1070.39 463.71"
					class="schema-domain-labels__line"
				/>
				<circle
					cx="1069.39"
				...
				<text
					...
					class="schema-domain-labels__text"
				>
					{{ domainData.architecture.shortLabel }}
				</text>

				<path
					d="M678.668 995.958L679.373 1042.95"
					class="schema-domain-labels__line"
				/>
				<circle
					cx="679.373"
				...
				<text
					...
					text-anchor="middle"
				>
					{{ domainData.ux.shortLabel }}
				</text>

				<path
					d="M297.52 583.136L250.709 578.926"
					class="schema-domain-labels__line"
				/>
				<circle
					cx="249.709"
				...
				<text
					...
					text-anchor="end"
				>
					{{ domainData.backend.shortLabel }}
				</text>

				<path
					d="M475.269 296.457L450.559 256.477"
					class="schema-domain-labels__line"
				/>
				<circle
					cx="449.559"
				...
				<text
					...
					text-anchor="end"
				>
					{{ domainData.maintenance.shortLabel }}
				</text>
			</g>
		</svg>
		...
		<aside class="schema-legend">
			<!-- Légende affichée au chargement -->
			<template v-if="!selectedSelection">
				...
				<div class="schema-legend__levels">
					<div class="schema-level-card">
						<span class="schema-level-card__badge schema-level-card__badge--essential">
							1
						</span>
						...
						<div>
							...
							<p>
								<span class="schema-level-card__number schema-level-card__number--essential">
									20
								</span>
					...
					<div class="schema-level-card">
						<span class="schema-level-card__badge schema-level-card__badge--advanced">
							2
						</span>
						...
						<div>
							...
							<p>
								<span class="schema-level-card__number schema-level-card__number--advanced">
									103
								</span>
				...
				<div class="schema-legend__domains">
					<span class="schema-domain-chip">
						<i
							class="schema-domain-chip__dot"
							:style="{ backgroundColor: domainData.strategy.color }"
						/>
						Stratégie
					</span>
					...
					<span class="schema-domain-chip">
						<i
							class="schema-domain-chip__dot"
							:style="{ backgroundColor: domainData.architecture.color }"
						/>
						Cadrage / Archi
					</span>
					...
					<span class="schema-domain-chip">
						<i
							class="schema-domain-chip__dot"
							:style="{ backgroundColor: domainData.ux.color }"
						/>
						UX / UI / Dev Front
					</span>
					...
					<span class="schema-domain-chip">
						<i
							class="schema-domain-chip__dot"
							:style="{ backgroundColor: domainData.backend.color }"
						/>
						Dev Back
					</span>
					...
					<span class="schema-domain-chip">
						<i
							class="schema-domain-chip__dot"
							:style="{ backgroundColor: domainData.maintenance.color }"
						/>
						Maintenance / Run
					</span>
			...
			<!-- Carte détaillée affichée après un clic -->
			<template
				v-else-if="selectedData"
			>
				<span class="schema-legend__level">
					Niveau #{{ selectedSelection.level }}
				</span>
				...
				<div class="schema-legend__domain">
					...
					<span>{{ selectedData.shortLabel }}</span>
				</div>
...
.schema-level-text__description--level-1 {
	fill: #166534;
	font-family: Inter, Arial, sans-serif;
	font-size: 20px;
	line-height: 1.2;
...
.schema-level-text__description--level-2 {
	fill: #fcfcfc;
	font-family: Inter, Arial, sans-serif;
	font-size: 20px;
}
