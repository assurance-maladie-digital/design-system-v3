...
{(() => {
  ...
  // Exclure les composants non pertinents des statistiques
  const validComponents = statusData.results.filter(item => 
    item.componentName !== 'AccessibilityProgressPage' && 
    item.componentName !== 'stories' && 
    item.componentName !== 'Tables'
  );
