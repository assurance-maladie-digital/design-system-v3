...
<Meta title="Démarrer/Guide Du Dev/Migration Vue2"/>

# Migrer vers Synapse

Ce guide vous explique comment migrer de la version Vue-dot (Vue2) du Design System vers Synapse.

## Vue-dot (Vue2) vers Synapse
#
...
- 1.1 - Téléchargez la stack de migration [Starter Kit Synapse](https://gitlab.cnqd.cnamts.fr/human/developpement/SKV3_X).

- 1.2 - Copier les pages de votre projet VueJS 2 dans le dossier pages du projet Starter Kit, les composants peuvent être placés dans le dossier components.

- 1.3 - Identifier les dépendances à conserver dans la nouvelle version, il faut choisir la version vue3 de la librairie et la version Nuxt si elle existe. Ajouter ces dépendances dans le nouveau projet.
...
3.1 - Conversion d’un store Vuex en Pinia
...
5.2 - Exécuter la commande `npx vue-class-migrator -d`  à la racine de votre projet.
...
5.4 - Si vous avez déclaré vos props en tant que mixins dans vos composants en utilisant le helper 'mixin' de `vue-class-componant`, vous devrez les migrer manuellement, les mixins ne sont pas supportés dans la syntaxe native VueJs.
...
7.3 - Dans les composants, quand un object est importé avec la fonction `$t`, il faut le remplacer par `$tm`.
