...
> **Non listés** (à traiter au cas par cas)&nbsp;: les tokens dont la valeur **diffère selon le thème** ou a **changé** lors de la refonte (ex. certaines nuances `*Subdued`/`accentSecondary`/`accentContrasted`, états `*OnDark`), et les tokens **interactifs** (restructurés en tokens d'`interaction`).
