...
<Meta title="Design Tokens/Correspondances couleurs" />
...
> **Non listés** (à traiter au cas par cas)&nbsp;: les tokens dont la valeur **diffère selon le thème** ou a **changé** lors de la refonte (ex. certaines nuances `*Subdued`/`accentSecondary`/`accentContrasted`, états `*OnDark`), et les tokens **interactifs** (restructurés en tokens d'`interaction`). La source de vérité reste `*LightTheme.ts`.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>colors.background.entreprise</code></td>
      <td><code>warning</code></td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>colors.overlay.onLight</code></td>
      <td><code>primary</code></td>
    </tr>
  </tbody>
</table>
