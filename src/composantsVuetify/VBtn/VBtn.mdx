...
La prop `icon` transforme le bouton en bouton rond ne contenant qu'une icône. Sans texte visible, le nom accessible doit être fourni manuellement via `aria-label` sur le `v-btn`, l'icône restant décorative.
