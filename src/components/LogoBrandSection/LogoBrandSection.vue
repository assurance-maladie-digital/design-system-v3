...
<template>
	<div :style="{ height: height }" class="vd-logo-brand-section d-flex flex-wrap">
		<component :is="logoContainerComponent"
			:to="logoContainerComponent === 'router-link' ? homeLink?.to : undefined"
			:href="logoContainerComponent === 'a' ? homeLink?.href : undefined" class="vd-home-link">
			<Logo :hide-signature="hideSignature" :hide-organism="isCompteAmeliMobile" :risque-pro="isRisquePro"
				:aria-label="homeLink?.ariaLabel + ' Retour vers accueil du site'" :avatar="avatar" :size="logoSize"
				:class="{ 'mr-2': avatar }" />
		</component>

		<slot>
			<svg v-if="showDivider" :width="dividerDimensions.width" :style="{ height: dividerDimensions.height }"
				:fill="dividerColor" focusable="false" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
				viewBox="0 0 22 64" class="vd-divider">
				<path d="M14.3 49.3c-.2 0-.4-.2-.4-.4V14.2c0-.2.2-.4.4-.4.3 0 .5.2.5.4v34.7c0 .2-.2.4-.5.4Z" />
			</svg>

			<component :is="secondaryLogoCtnComponent" v-if="secondaryLogo" :aria-label="secondaryLogoLabel"
				:to="secondaryLogoCtnComponent === 'router-link' ? homeLink?.to : undefined"
				:href="secondaryLogoCtnComponent === 'a' ? homeLink?.href : undefined" class="vd-home-link">
				<img :src="secondaryLogo.src" :alt="secondaryLogo.alt">
			</component>

			<div v-else-if="showBrandContent" class="vd-title-container d-flex justify-center flex-column text-primary">
				<slot name="brand-content">
					<SyHeading v-if="service.title" :class="{
						'vd-compte-entreprise-title': isCompteEntreprise,
					}" class="vd-title text-caption text-md-subtitle-1 font-weight-medium" :level="1">
					...
					<SyHeading v-if="showServiceSubTitle" class="vd-title text-caption" :level="2">
						{{ service.subTitle }}
					</SyHeading>
				</slot>
			</div>
