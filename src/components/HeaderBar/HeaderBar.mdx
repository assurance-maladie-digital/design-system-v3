...
Le contenu du slot prend tout l'espace disponible en en largeur. Pour que le contenu soit de même dimension que le header, il faut lui appliquer une largeur maximale de `1712px`.
...
<Canvas
	of={HeaderBarStories.PrependSlot}
	source={{
language: 'html',
code:`
<template>
...
<Canvas
	of={HeaderBarStories.AppendSlot}
	source={{
		language: 'html',
		code:`
<template>
...
Le slot `menu` permet d'ajouter un menu au header. Il est recommandé d'utiliser le composant `HeaderBurgerMenu` du design system.
...
Voir le composant `HeaderBurgerMenu` pour un exemple d'usage..

### Logo

Le slot `logo` permet d’écraser le logo par défaut du header. Il est déconseillé de l'utiliser. Si cela n'est pas possible l'intégration d'un nouveau logo doit se faire en respectant les guidelines de la charte graphique.
...
<Canvas
	of={HeaderBarStories.CustomLogo}
		source={{
language: 'html',
code:`
<template>
...
Le slot `custom-brand` permet d'ajouter un contenu personnalisé à la place du logo. Il est possible d'ajouter plusieurs éléments dans ce slot.

<Canvas
	of={HeaderBarStories.CustomBrand}
	source={{
		language: 'html',
		format: 'dedent',
		code:`
			<template>
				<HeaderBar>
					<template #logo-brand-content>
						<svg data-v-a3ce97de="" width="22" height="64" fill="#0c419a" role="img" focusable="false" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 64" class="vd-divider"><path data-v-a3ce97de="" d="M14.3 49.3c-.2 0-.4-.2-.4-.4V14.2c0-.2.2-.4.4-.4.3 0 .5.2.5.4v34.7c0 .2-.2.4-.5.4Z"></path></svg>
						<div>
							<div>
								<div style="font-size: 1.15rem;">Brand <span style="color: red">custom</span></div>
								<div style="font-size: 0.75rem;">un service de l'assurance maladie</div>
							</div>
						</div>
					</template>
				</HeaderBar>
