...
describe('ChipList', () => {
	...
	it('applique les bonnes classes selon l\'état', () => {
		...
		expect(chips[0]?.classes()).toContain('bg-success-variant')
		expect(chips[1]?.classes()).toContain('bg-info-variant')
		expect(chips[2]?.classes()).toContain('bg-warning-variant')
		expect(chips[3]?.classes()).toContain('bg-error-variant')
