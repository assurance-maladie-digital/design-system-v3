...
<CriteriaSection>
    <CriteriaCard title="Parcours recommandé">
        <ul>
            <li>Privilégier un formulaire complet avec une action de validation explicite.</li>
            <li>Configurer <code>lock-after-selection="false"</code> pour ne pas bloquer l’utilisateur dès la première sélection.</li>
            <li>Ce comportement permet de corriger une erreur avant l’envoi, ce qui réduit les risques pour les personnes qui naviguent au clavier, à la voix ou avec un lecteur d’écran.</li>
            <li>Afficher le message de confirmation après la validation du formulaire, pas dès le premier choix.</li>
        </ul>
    </CriteriaCard>
    ...
    <CriteriaCard title="Message de confirmation">
        <ul>
            <li>Dans le parcours recommandé, le message s’affiche après validation du formulaire (ex : <code>Merci pour votre réponse</code>).</li>
            <li>Ce message doit être annoncé automatiquement, sans déplacer le focus.</li>
            <li>On utilise <code>SyAlert</code> avec <code>role="status"</code> et <code>aria-live="polite"</code>.</li>
            <li>Ne pas utiliser <code>role="alert"</code> pour ce type de message sauf si l’information est réellement urgente.</li>
        </ul>
    </CriteriaCard>
...
<DemoSection componentName="RatingPicker">
    <Canvas of={Stories.AccessibilityBestPractices} />
</DemoSection>
...
<BestPracticesSection>
    <ul>
        ...
        <li>Laisser la sélection modifiable jusqu’à la validation du formulaire avec <code>lock-after-selection="false"</code>.</li>
        ...
        <li>Utiliser <code>SyAlert</code> avec <code>role="status"</code> et <code>aria-live="polite"</code> pour le message de confirmation dans le cas général.</li>
