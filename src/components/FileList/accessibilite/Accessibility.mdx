  ...
  <CriteriaSection>
    <CriteriaCard title="Structure sémantique">
      <ul>
        ...
        <li><strong>Aria-label</strong> : Les <code>aria-label</code> sont construits en combinant le label résolu (via <code>resolvedImportLabel</code>, <code>resolvedSeeLabel</code>, <code>resolvedDeleteLabel</code>) avec le titre ou le nom du fichier correspondant.</li>
        <li><strong>SeeLabel, ImportLabel, DeleteLabel</strong> : Utilisation de ces propriétés pour fournir des étiquettes claires et descriptives pour les actions associées à chaque fichier. À défaut, ce sont les locales (<code>see</code>, <code>import</code>, <code>delete</code>) qui sont utilisées.</li>
