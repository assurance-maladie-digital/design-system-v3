...
const getTabs = (): AmeliproMultipleFoldingCardItem[] => [
	{
		id: 'the-item-id-1',
		title: 'The item title 1',
	},
	{
		id: 'the-item-id-2',
		title: 'The item title 2',
	},
	{
		id: 'the-item-id-3',
		title: 'The item title 3',
	},
	{
		id: 'the-item-id-4',
		title: 'The item title 4',
	},
	{
		id: 'the-item-id-5',
		title: 'The item title 5',
	},
]
	...
	describe('Events', () => {
		...
		it('should react to button event and update attributes', async () => {
			...
			if (btns[0]) {
				...
				expect(btns[0].attributes('id')).toBe('the-item-id-1-title')
				expect(btns[0].attributes('aria-controls')).toBe('the-item-id-1')
			...
			if (btns[1]) {
				...
				expect(btns[1].attributes('id')).toBe('the-item-id-2-title')
				expect(btns[1].attributes('aria-controls')).toBe('the-item-id-2')
				expect(btns[1].attributes('aria-expanded')).toBe('false')
			}

			await btns[0]?.trigger('click')
			expect(wrapper.emitted('tab-clicked')).toStrictEqual([['the-item-id-1']])
			if (btns[0]) {
			...
			expect(wrapper.emitted('tab-clicked')).toStrictEqual([['the-item-id-1'], ['the-item-id-2']])
			if (btns[0]) {
			...
			await btns[0]?.trigger('click')
			expect(wrapper.emitted('tab-clicked')).toStrictEqual([['the-item-id-1'], ['the-item-id-2'], ['the-item-id-1']])
			if (btns[0]) {
				expect(btns[0].attributes('class')).toBe('d-inline-flex align-center amelipro-card--multi-folding__btn classic-btn active-btn')
				expect(btns[0].attributes('aria-expanded')).toBe('true')
			}
