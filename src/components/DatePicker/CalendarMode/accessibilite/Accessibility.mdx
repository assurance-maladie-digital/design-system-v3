...
<CriteriaSection>
	<CriteriaCard title="Structure sémantique" icon="">
		<ul>
			...
			<li><strong>Panneau</strong> : calendrier exposé dans un vrai <code>dialog</code> modal, contenant la grille de jours et ses contrôles.</li>
			<li><strong>Relations</strong> : <code>aria-controls</code> relie l’enveloppe activatrice du champ readonly au <code>dialog</code> lorsqu’il est ouvert; le <code>dialog</code> est nommé par un titre stable « Calendrier » via <code>aria-labelledby</code>; <code>aria-owns/expanded/haspopup</code> ne sont pas surchargés sur l’input natif.</li>
	...
	<CriteriaCard title="Ouverture et focus" icon="">
		<ul>
			<li><strong>Ouverture clavier</strong> : <kbd>Entrée</kbd> ou <kbd>Espace</kbd> sur le champ ouvre le calendrier; l’icône déclenche l’ouverture avec la souris.</li>
			<li><strong>Fermeture</strong> : <kbd>Échap</kbd> ferme et valide; un clic extérieur ferme aussi le panneau; le focus est ensuite rendu au champ.</li>
	...
	<CriteriaCard title="Navigation clavier dans le calendrier" icon="">
		<ul>
			...
			<li><strong>Home/End</strong> : vont au premier/dernier jour de la semaine affichée.</li>
	...
	<CriteriaCard title="Messages et annonces" icon="">
		<ul>
			<li><strong>Live region</strong> : le mois/année affichés sont annoncés via une unique région de statut dédiée, pour éviter les annonces en double.</li>
...
<BestPracticesSection>
- Fournir un <code>label</code> explicite; utiliser <code>placeholder</code> uniquement comme aide complémentaire, jamais comme seul libellé.
- Conserver <code>format</code> et le texte d’aide cohérents avec la valeur attendue; utiliser <code>hint</code> pour expliquer le format métier.
- Laisser <code>isValidateOnBlur</code> à <code>true</code> si les erreurs ne doivent apparaître qu’après la sortie du champ; réserver <code>validateOnSubmit()</code> à la soumission.
- En lecture seule, le champ n’est pas éditable et le calendrier ne doit pas être proposé; pour un champ sélectionnable mais non saisissable, utiliser le mode calendrier normal, pas <code>readonly</code>.
...
- Ne pas ajouter <code>role="combobox"</code> au mode calendrier seul : ce mode ne permet pas la saisie d’une valeur.
</BestPracticesSection>

<ResourcesSection>
- Stories DatePicker : cas standard, plage, lecture seule.
- Tests : <code>CalendarMode/tests/DatePicker.spec.ts</code>, <code>CalendarMode/tests/DatePicker.a11y.spec.ts</code> et <code>composables/tests/useDatePickerFocusTrap.a11y.spec.ts</code> couvrent l’ouverture, la fermeture, le focus et la navigation clavier.
