...
<template>
	<div class="date-picker-container">
		<template v-if="props.noCalendar">
			<DateTextInput ref="dateTextInputRef" v-model="textInputValue"
				:class="[getMessageClasses(), 'label-hidden-on-focus']" :label="labelWithAsterisk || ''"
				:placeholder="props.placeholder" :format="props.format" :date-format-return="props.dateFormatReturn"
				:required="props.required" :display-range="props.displayRange" :display-icon="props.displayIcon"
				:display-append-icon="props.displayAppendIcon" :display-prepend-icon="props.displayPrependIcon"
				:custom-rules="props.customRules" :custom-warning-rules="props.customWarningRules"
				:disabled="props.disabled" :no-icon="props.noIcon" :is-outlined="props.isOutlined"
				:readonly="props.readonly" :title="props.title || props.placeholder || undefined" :width="props.width"
				:disable-error-handling="props.disableErrorHandling" :show-success-messages="props.showSuccessMessages"
				:bg-color="props.bgColor" :density="props.density" :hide-details="props.hideDetails"
				:period="props.period" :auto-clamp="props.autoClamp" :display-asterisk="props.displayAsterisk"
				:is-validate-on-blur="props.isValidateOnBlur" :hint="props.hint" :persistent-hint="props.persistentHint"
				@update:model-value="handleDateTextInputUpdate" @date-selected="handleDateTextInputSelection"
				@blur="handleInputBlur" @focus="emit('focus')" />
		</template>
		<template v-else-if="props.useCombinedMode">
			<ComplexDatePicker ref="complexDatePickerRef" :model-value="props.modelValue" :format="props.format"
				:date-format-return="props.dateFormatReturn" :required="props.required"
				:custom-rules="props.customRules" :custom-warning-rules="props.customWarningRules"
				:disabled="props.disabled" :readonly="props.readonly" :is-outlined="props.isOutlined"
				:display-icon="props.displayIcon" :display-append-icon="props.displayAppendIcon"
				:display-prepend-icon="props.displayPrependIcon" :no-icon="props.noIcon"
				:disable-error-handling="props.disableErrorHandling" :show-success-messages="props.showSuccessMessages"
				:bg-color="props.bgColor" :density="props.density" :hide-details="props.hideDetails"
				:display-range="props.displayRange" :display-weekend-days="props.displayWeekendDays"
				:display-today-button="props.displayTodayButton" :display-holiday-days="props.displayHolidayDays"
				:display-asterisk="props.displayAsterisk" :show-week-number="props.showWeekNumber"
				:is-birth-date="props.isBirthDate || props.birthDate" :text-field-activator="props.textFieldActivator"
				:title="props.title" :period="period" :auto-clamp="props.autoClamp" :label="props.label"
				:placeholder="props.placeholder" :is-validate-on-blur="props.isValidateOnBlur" :hint="props.hint"
				:persistent-hint="props.persistentHint" @update:model-value="emit('update:modelValue', $event)"
				@focus="emit('focus')" @blur="emit('blur')" @date-selected="emit('date-selected', $event)" />
		</template>
		<template v-else>
			<VMenu v-if="!props.noCalendar" v-model="isDatePickerVisible" activator="parent" :min-width="0"
				location="bottom" :close-on-content-click="false" :open-on-click="false" scroll-strategy="reposition"
				transition="fade-transition" attach="body" :offset="[-20, 5]">
				<template #activator="{ props: menuProps }">
					<SyTextField v-bind="menuProps" ref="dateCalendarTextInputRef" v-model="displayFormattedDate"
						:append-icon="displayIcon && displayAppendIcon ? 'calendar' : undefined"
						:class="[getMessageClasses(), 'label-hidden-on-focus']" :error-messages="errorMessages"
						:warning-messages="warningMessages"
						:success-messages="props.showSuccessMessages ? successMessages : []" :disabled="props.disabled"
						:disable-click-button="false" :readonly="true" :label="labelWithAsterisk"
						:placeholder="props.placeholder" :no-icon="props.noIcon"
						:prepend-icon="displayIcon && !displayAppendIcon ? 'calendar' : undefined"
						:variant-style="props.isOutlined ? 'outlined' : 'underlined'" color="primary"
						:show-success-messages="props.showSuccessMessages" :bg-color="props.bgColor"
						:density="props.density" :hide-details="props.hideDetails"
						:display-asterisk="props.displayAsterisk" :is-clearable="!props.readonly"
						:auto-clamp="props.autoClamp" :title="props.title" :hint="props.hint"
						:persistent-hint="props.persistentHint" @click="openDatePickerOnClick"
						@focus="openDatePickerOnFocus" @blur="handleInputBlur" @keydown="handleInputKeydown"
						@update:model-value="updateSelectedDates" @prepend-icon-click="openDatePickerOnIconClick"
						@append-icon-click="openDatePickerOnIconClick" />
				</template>
				<VDatePicker v-if="isDatePickerVisible && !props.noCalendar" ref="datePickerRef" v-model="selectedDates"
					color="primary" :first-day-of-week="1" :multiple="props.displayRange ? 'range' : false"
					:show-adjacent-months="true" :show-week="props.showWeekNumber" :view-mode="currentViewMode"
					:class="displayWeekendDays ? 'weekend' : ''" :max="maxDate" :min="minDate"
					:display-holiday-days="props.displayHolidayDays" @update:view-mode="handleViewModeUpdate"
					@update:month="onUpdateMonth" @update:year="onUpdateYear" @click:date="updateSelectedDates"
					@update:model-value="updateDisplayFormattedDate" @focus="markHolidayDays"
					@update:month-year="markHolidayDays">
					<template #title>
						Sélectionnez une date
					</template>
					<template #header>
						<SyHeading class="mx-auto my-auto ml-5 mb-4" :level="3">
							{{ selectedDates ? displayedDateString : headerDate }}
						</SyHeading>
					</template>
					<template v-if="props.displayTodayButton" #actions>
						<div class="d-flex justify-center align-center w-100">
							<v-btn v-if="props.displayTodayButton" size="x-small" color="primary"
								:title="DATE_PICKER_MESSAGES.BUTTON_TODAY"
								class="date-picker__today-button my-2 pa-2 mt-2" :ripple="false"
								@click="handleSelectToday">
								<SyIcon size="16px" decorative :icon="mdiCalendarMonthOutline" />
