...
<BestPracticesSection>
- Fournir un <code>label</code> explicite; utiliser <code>placeholder</code> uniquement comme aide complémentaire.
- Conserver <code>format</code>, <code>dateFormatReturn</code> et le <code>hint</code> cohérents pour éviter une valeur affichée différente de la valeur émise.
- Laisser <code>isValidateOnBlur</code> à <code>true</code> si les erreurs et le succès doivent être annoncés au blur; utiliser <code>validateOnSubmit()</code> pour le contrôle de soumission.
- Avec <code>textFieldActivator</code>, s’assurer que toute la zone d’activation reste compréhensible au clavier et que l’icône n’est pas le seul point d’entrée.
- Si <code>readonly</code>, le champ ne doit pas être utilisé pour saisir ou effacer une date; ne pas le présenter comme un combobox éditable.
