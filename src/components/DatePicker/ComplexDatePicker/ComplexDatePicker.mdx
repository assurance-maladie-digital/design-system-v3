...
<Canvas of={GestionDesFormatsStories.AutoFormattingInput}/>

## Désactivation de l'interaction avec le calendrier

La propriété `noCalendar` permet d'utiliser le champ de saisie avec formatage automatique sans que le calendrier n'apparaisse au clic sur l'input ou sur l'icône. Cette option est utile pour les cas où on souhaite uniquement la saisie manuelle.
...
Les bornes `period.min` et `period.max` doivent rester exprimées dans le même format que la prop `format`, car elles sont fournies directement au calendrier.

<Canvas of={GestionDesFormatsStories.CustomDateFormat}/>
...
>
    ...
    <ul>
        ...
        <li>Le format de la prop <code>period</code> doit rester cohérent avec la prop <code>format</code> utilisée par le composant.</li>
    </ul>
</div>
...
<Canvas of={ValidationStories.WithValidation}/>
...
### reset
- **Description** : Efface la sélection, les messages de validation et referme le calendrier
