...
### Icône avec le rôle button

<Canvas of={SyIconStories.WithRoleButton} />
> **Important :** Si l'icône doit être interactive (`role="button"`), privilégiez l'utilisation du composant [SyIconButton](/docs/composants-boutons-syiconbutton--docs) plutôt que SyIcon.

 SyIconButton est conçu pour les interactions utilisateur et intègre les comportements d'accessibilité ainsi que les états visuels tels que hover, pressed et focus. Le composant SyIcon est destiné à l'affichage d'une icône simple, non interactive.
