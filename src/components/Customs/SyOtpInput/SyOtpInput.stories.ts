...
import SyOtpInput from '@/components/Customs/SyOtpInput/SyOtpInput.vue'
import { ref, watch } from 'vue'
import { fn } from '@storybook/test'

// Interface pour typer correctement le composant SyOtpInput avec sa méthode validateOnSubmit
interface SyOtpInputInstance {
	validateOnSubmit: () => boolean
}

const meta = {
	title: 'Composants/Formulaires/SyOtpInput',
	component: SyOtpInput,
	decorators: [
		() => ({
	...
	parameters: {
		...
		docs: {
			description: {
				component: `SyOtpInput est un composant de case à cocher tri-état qui étend le composant VCheckbox de Vuetify avec des fonctionnalités supplémentaires comme la validation personnalisée et l'état indéterminé.`,
			},
		},
	},
	argTypes: {
...
} as Meta<typeof SyOtpInput>

export default meta

type Story = StoryObj<typeof SyOtpInput>
...
export const Default: Story = {
	parameters: {
		sourceCode: [
			{
				name: 'Template',
				code: `<SyOtpInput v-model="checked" label="Case à cocher" />`,
			},
		],
	},
	args: {
	...
	render: args => ({
		components: { SyOtpInput },
		setup() {
			const checked = ref(false)
			return { args, checked }
		},
		template: `<SyOtpInput v-model="checked" v-bind="args" label="Case à cocher" />`,
	}),
}
...
export const Required: Story = {
	...
	render: args => ({
		...
		template: `<SyOtpInput v-model="checked" v-bind="args" label="Case à cocher obligatoire" :is-validate-on-blur="false" />`,
	}),
	parameters: {
		sourceCode: [
			{
				name: 'Template',
				code: `<SyOtpInput v-model="checked" label="Case à cocher obligatoire" required :is-validate-on-blur="false" />`,
			},
		],
		docs: {
			description: {
...
export const Indeterminate: Story = {
	...
	render: args => ({
		...
		template: `<SyOtpInput v-model="checked" :indeterminate="indeterminate" v-bind="args" label="Case à cocher indéterminée" />`,
	}),
	parameters: {
		sourceCode: [
			{
...
<template>
  <div>
    ...
    <div style="margin-left: 24px;">
      <SyOtpInput v-model="childChecked1" id="child-1" label="Enfant 1" />
      <SyOtpInput v-model="childChecked2" id="child-2" label="Enfant 2" />
      <SyOtpInput v-model="childChecked3" id="child-3" label="Enfant 3" />
    </div>
  </div>
</template>
...
export const DisabledState: Story = {
	...
	render: args => ({
		...
		template: `<SyOtpInput v-model="checked" v-bind="args" label="Case à cocher désactivée" />`,
	}),
	parameters: {
		sourceCode: [
			{
				name: 'Template',
				code: `<SyOtpInput v-model="checked" disabled label="Case à cocher désactivée" />`,
			},
		],
		docs: {
			description: {
...
export const ReadonlyState: Story = {
	...
	render: args => ({
		...
		template: `<SyOtpInput v-model="checked" v-bind="args" label="Case à cocher en lecture seule" />`,
	}),
	parameters: {
		sourceCode: [
			{
				name: 'Template',
				code: `<SyOtpInput v-model="checked" readonly label="Case à cocher en lecture seule" />`,
			},
		],
		docs: {
			description: {
		...
		sourceCode: [
			{
				...
				code: `
                <div>
                <SyOtpInput v-model="checked1" label="Densité par défaut" />
                <SyOtpInput v-model="checked2" label="Densité confortable" density="comfortable" />
                <SyOtpInput v-model="checked3" label="Densité compacte" density="compact" />
                </div>`,
			},
		],
		docs: {
...
Le composant SyOtpInput prend en charge différentes densités pour s'adapter à différents contextes d'interface utilisateur.
				`,
			},
		},
	},
		...
		sourceCode: [
			{
				...
				code: `
<div>
  <SyOtpInput v-model="checked1" label="Couleur primaire (par défaut)" />
  <SyOtpInput v-model="checked2" label="Couleur secondaire" color="secondary" />
  <SyOtpInput v-model="checked3" label="Couleur succès" color="success" />
  <SyOtpInput v-model="checked4" label="Couleur erreur" color="error" />
  <SyOtpInput v-model="checked5" label="Couleur avertissement" color="warning" />
</div>`,
			},
		],
		docs: {
...
Le composant SyOtpInput peut être personnalisé avec différentes couleurs pour s'adapter à votre thème.
				`,
			},
		},
	},
...
const checkbox = ref<SyOtpInputInstance | null>(null)
const checkbox2 = ref<SyOtpInputInstance | null>(null)
const checked = ref(false)
const checked2 = ref(false)
