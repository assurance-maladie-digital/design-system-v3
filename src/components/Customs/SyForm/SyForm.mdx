...
{/* func-version-start */}
<p className="func-version-badge">Dernière mise à jour fonctionnelle : V1.0.16 - 12/12/2025</p>
{/* func-version-end */}
{/* a11y-version-start */}
<p className="a11y-version-badge">Dernière mise à jour accessibilité : V1.0.21 - 25/02/2026</p>
{/* a11y-version-end */}
...
## État de validation (`v-model`)

`SyForm` expose la validité globale du formulaire via un `v-model` **à trois états**
(`boolean | null`). C'est pratique pour piloter, par exemple, l'état activé d'un bouton de
soumission.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Tous les champs ont été évalués et sont valides.</td>
    </tr>
    <tr>
      <td><code>false</code></td>
      <td>Au moins un champ est invalide.</td>
    </tr>
    <tr>
      <td><code>null</code></td>
      <td>État <strong>inconnu</strong> : au moins un champ n'a pas encore été évalué (vierge), ou le formulaire ne contient aucun champ. À ne pas confondre avec « invalide ».</td>
    </tr>
  </tbody>
</table>

Le `v-model` ne vaut `true` que lorsque **tous** les champs ont été validés et passent : un
champ optionnel non encore touché maintient l'état à `null` (et non `false`). Pour n'activer
un bouton que sur un formulaire pleinement valide, comparez donc explicitement à `true` :

<Source dark code={'\n<script setup lang="ts">\nimport { ref } from "vue"\nimport { SyForm, SyTextField } from "@cnamts/synapse"\n\nconst valid = ref(null) // true | false | null\nconst name = ref("")\n</script>\n\n<template>\n  <SyForm v-model="valid">\n    <div class="d-flex flex-column gap-4">\n      <SyTextField v-model="name" label="Nom" required />\n      <!-- Actif uniquement quand tout est validé (valid === true) -->\n      <v-btn type="submit" color="primary" :disabled="valid !== true">Soumettre</v-btn>\n    </div>\n  </SyForm>\n</template>'} />
