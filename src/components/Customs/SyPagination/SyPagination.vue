...
<template>
	<div class="sy-pagination">
		...
		<nav v-if="totalPages > 1" class="pagination" :aria-labelledby="uniqueId" :aria-controls="ariaControls">
			<SyHeading :id="uniqueId" class="d-sr-only" :level="2">
				{{ `${label}` }}
			</SyHeading>
			<ul class="list">
				<!-- First page button (optional) -->
				<li v-if="$slots['first-page']">
					<a href="#" class="first-page" :class="{ 'disabled': !hasPrevious }"
						:aria-disabled="!hasPrevious ? 'true' : undefined" @click.prevent="firstPage">
				...
				<!-- Previous button -->
				<li>
					<a href="#" class="previous-page" :class="{ 'disabled': !hasPrevious }"
						:aria-disabled="!hasPrevious ? 'true' : undefined" @click.prevent="previousPage">
						<slot name="previous">
							<span>Précédent</span>
						</slot>
					</a>
				...
				<!-- First page -->
				<li>
					<a href="#" class="list-first" :aria-current="modelValue === 1 ? 'page' : undefined"
						@click.prevent="goToPage(1)">
						<slot name="page-number" :page="1">
							{{ 1 }}
						</slot>
					</a>
				</li>
				...
				<li v-if="visiblePageNumbers.includes('ellipsis-start')" class="ellipsis-item">
					<a href="#" class="ellipsis" @click.prevent>…</a>
				</li>

				<!-- Middle pages -->
				<li v-for="pageNum in middlePages" :key="pageNum">
					<a href="#" :aria-current="modelValue === pageNum ? 'page' : undefined"
						@click.prevent="goToPage(pageNum)">
						<slot name="page-number" :page="pageNum">
							{{ pageNum }}
						</slot>
					</a>
				</li>
				...
				<li v-if="visiblePageNumbers.includes('ellipsis-end')" class="ellipsis-item">
					<a href="#" class="ellipsis" @click.prevent>…</a>
				</li>

				<!-- Last page (if not already shown) -->
				<li v-if="totalPages > 1">
					<a href="#" class="list-last" :aria-current="modelValue === totalPages ? 'page' : undefined"
						@click.prevent="goToPage(totalPages)">
						<slot name="page-number" :page="totalPages">
							{{ totalPages }}
						</slot>
					</a>
				</li>
				...
				<!-- Next button -->
				<li>
					<a href="#" class="next-page" :class="{ 'disabled': !hasNext }"
						:aria-disabled="!hasNext ? 'true' : undefined" @click.prevent="nextPage">
						<slot name="next">
							<span>Suivant</span>
						</slot>
					</a>
				...
				<li v-if="$slots['last-page']">
					<a href="#" class="last-page" :class="{ 'disabled': !hasNext }"
						:aria-disabled="!hasNext ? 'true' : undefined" @click.prevent="lastPage">
						<slot name="last-page">
							<span>»</span>
						</slot>
					</a>
