...
>
	...
	<CriteriaSection>
		<CriteriaCard title="Rôle ARIA et structure sémantique">
			<ul>
				<li><strong>Rôle par défaut</strong> : Le composant utilise <code>role="alert"</code> pour annoncer automatiquement les messages importants aux lecteurs d'écran</li>
				<li><strong>Annonce personnalisable</strong> : Les props <code>role</code> et <code>ariaLive</code> permettent d’adapter la priorité d’annonce de <code>SyAlert</code></li>
				<li><strong>Messages non urgents</strong> : Utilisez <code>role="status"</code> avec <code>aria-live="polite"</code> pour une confirmation ou une information qui ne doit pas interrompre l’utilisateur</li>
	...
	<BestPracticesSection>
		<ul>
			...
			<li><strong>Priorité d’annonce</strong> : Gardez <code>role="alert"</code> pour les messages urgents, et préférez <code>role="status"</code> avec <code>aria-live="polite"</code> pour les confirmations non bloquantes</li>
