...
Les filtres texte et numériques acceptent notamment `maxlength` pour limiter la saisie. Les autres options de champ disponibles sont `variant`, `density`, `hideDetails`, `backgroundColor`, `clearable`, `disableErrorHandling` et `debounceTime`.

<Source dark code={`
const headers = [
  { title: 'Nom', key: 'lastname', filterable: true, filterType: 'text' },
  { title: 'Référence', key: 'reference', filterable: true, filterType: 'number' },
]
...
`maxlength` limite la saisie dans le champ ; les contraintes métier restent à valider côté application et côté serveur. Voir d'autres exemples de filtres dans les stories.
