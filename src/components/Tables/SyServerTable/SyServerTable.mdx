...
{/* func-version-start */}
<p className="func-version-badge">Dernière mise à jour fonctionnelle : V1.1.0 - 29/05/2026</p>
{/* func-version-end */}
{/* a11y-version-start */}
...
L'éditeur d'une cellule peut être personnalisé via le slot `#edit.<columnKey>` (reçoit `{ item, value, update }`).

**Évènements** : `edit(item)` · `save(updated, original)` · `cancel(item)` · `delete(item)`.

<Canvas story={{height: '350px'}} of={RowEditing.Default} />

### Actions groupées (suppression et édition en masse)

En s'appuyant sur la sélection (`show-select`), une barre d'actions groupées apparaît dès qu'au moins une ligne est cochée :

- **`show-delete-selected`** ajoute un bouton de suppression en masse → émet `delete-multiple(items)`.
- **`show-edit-selected`** ajoute un bouton d'édition groupée : une boîte de dialogue édite **chaque ligne sélectionnée séquentiellement** (navigation par flèches ‹ ›), pré-remplie avec ses propres données. À la validation, `save-multiple(items)` reçoit **uniquement les lignes modifiées** (objets complets).
...
La barre est personnalisable via le slot `#bulk-actions` (`{ selected, count, clearSelection, deleteSelected, editSelected }`) et le formulaire d'édition via `#bulk-edit-form`. Les libellés se personnalisent avec les props `bulk-selected-label`, `bulk-edit-title` et `bulk-edit-position-label`.
