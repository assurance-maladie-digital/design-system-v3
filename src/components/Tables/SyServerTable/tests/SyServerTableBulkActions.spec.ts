...
describe('SyServerTable — barre de sélection multiple (actions pilotées par le projet)', () => {
	beforeAll(() => {
	...
	it('n\'affiche pas la barre si aucun slot #bulk-actions n\'est fourni', async () => {
		const wrapper = await mountTable({ modelValue: [1, 2] }, {})
		expect(wrapper.find(BAR).exists()).toBe(false)
	})

	it('affiche la barre avec le bon décompte quand des lignes sont sélectionnées', async () => {
		const wrapper = await mountTable({ modelValue: [1, 2] })
		expect(wrapper.find(BAR).exists()).toBe(true)
		expect(wrapper.find(BAR).text()).toContain('2 éléments sélectionnés')
	})

	it('accorde le singulier pour une seule sélection', async () => {
		const wrapper = await mountTable({ modelValue: [1] })
		expect(wrapper.find(BAR).text()).toContain('1 élément sélectionné')
	})

	it('expose { selected, count, clearSelection } au slot #bulk-actions', async () => {
	...
	it('les checkboxes de sélection de ligne ont un libellé accessible (même cochées)', async () => {
		const wrapper = await mountTable({ modelValue: [1, 2, 3] })
