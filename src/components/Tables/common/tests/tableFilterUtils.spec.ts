...
describe('tableFilterUtils', () => {
	describe('filterItems', () => {
		...
		// Tests for text filter rules
		it('should support wildcard * in text filters', () => {
			const filters: FilterOption[] = [
				{ key: 'name', value: 'J*', type: 'text' },
			]
			const result = filterItems(testItems, filters)
			// With our current implementation, this will match John, Jane, and Bob Johnson (contains 'J')
			expect(result.length).toBeGreaterThanOrEqual(2)
			expect(result.map(item => item.id)).toContain(1) // John Doe
			expect(result.map(item => item.id)).toContain(2) // Jane Smith
		})

		it('should support single character wildcard ? in text filters', () => {
			const filters: FilterOption[] = [
				{ key: 'name', value: 'Joh? Doe', type: 'text' },
			]
			const result = filterItems(testItems, filters)
			expect(result.length).toBe(1)
			expect(result[0].id).toBe(1) // John Doe
		})

		it('should support case sensitive search with quotes', () => {
			// Case sensitive search should match exact case
			const filters1: FilterOption[] = [
				{ key: 'name', value: '"John"', type: 'text' },
			]
			const result1 = filterItems(testItems, filters1)
			// The implementation uses includes() which will find 'John' in 'John Doe'
			expect(result1.length).toBeGreaterThanOrEqual(1)
			expect(result1.map(item => item.id)).toContain(1) // John Doe
			...
			expect(result2.length).toBe(0) // Should not match because of case sensitivity
		})

		it('should support empty value search with <>?*', () => {
			// Add an item with empty name
			const itemsWithEmpty = [...testItems, { id: 6, name: '', age: 50, department: 'IT', status: 'Active' }]

			const filters: FilterOption[] = [
				{ key: 'name', value: '<>?*', type: 'text' },
			]
			const result = filterItems(itemsWithEmpty, filters)
			expect(result.length).toBe(1)
			expect(result[0].id).toBe(6) // Item with empty name
		})

		it('should support exact length search with =????', () => {
			const filters: FilterOption[] = [
				{ key: 'name', value: '=????', type: 'text' },
			]
			const result = filterItems(testItems, filters)
			expect(result.length).toBe(0) // No names with exactly 4 characters

			// Add an item with a 4-character name
			const itemsWithShortName = [...testItems, { id: 7, name: 'Alex', age: 28, department: 'IT', status: 'Active' }]
			const result2 = filterItems(itemsWithShortName, filters)
			expect(result2.length).toBe(1)
			expect(result2[0].id).toBe(7) // Alex (4 characters)
		})

		it('should support alphabetical comparison with >', () => {
			const filters: FilterOption[] = [
				{ key: 'name', value: '>T', type: 'text' },
			]
			const result = filterItems(testItems, filters)
			expect(result.length).toBe(1)
			expect(result[0].id).toBe(5) // Tom Wilson (starts with T, which is after 'T' alphabetically)
		})
	})
})
