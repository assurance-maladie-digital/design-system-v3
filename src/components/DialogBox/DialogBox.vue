...
<template>
	<VDialog v-model="dialog" :scrim="props.draggable ? 'transparent' : true" v-bind="$attrs" :width="props.width"
		:persistent="props.persistent" :retain-focus="false" aria-modal="true" class="sy-dialog-box"
		:aria-labelledby="id" @keydown.tab="handleFocus">
		<VCard v-bind="options.card" ref="dialogContent" :class="{
			...
			<VCardTitle class="sy-dialog-box-title" v-bind="options.cardTitle"
			...
			>
				...
				<VBtn v-if="!props.persistent" class="sy-dialog-box-close-btn" v-bind="options.closeBtn"
					:aria-label="locales.closeBtn" @click.stop="dialog = false">
					<SyIcon :icon="closeIcon" :decorative="true" v-bind="options.icon" />
				</VBtn>
			</VCardTitle>

			<div class="px-6" v-bind="options.cardText">
				<slot />
			</div>

			<div v-if="!props.hideActions" v-bind="options.cardActions" :class="options.actionsSlot?.class"
				class="sy-dialog-box-actions-ctn">
				<VSpacer v-bind="options.actionsSpacer" />

				<slot name="actions">
					<template v-if="display.xs.value">
						<VBtn v-bind="options.confirmBtn" data-test-id="confirm-btn" @click="$emit('confirm')">
					...
					<template v-else>
						<VBtn class="sy-dialog-box-cancel-btn" v-bind="options.cancelBtn" @click="$emit('cancel')">
							{{ props.cancelBtnText }}
						</VBtn>

						<VBtn v-bind="options.confirmBtn" ref="confirmBtn" class="sy-dialog-box-confirm-btn"
							:class="props.autofocusValidateBtn ? 'sy-dialog-box-confirm-btn--autofocus' : ''"
							data-test-id="confirm-btn" @click="$emit('confirm')">
							{{ props.confirmBtnText }}
						</VBtn>
					</template>
				</slot>
