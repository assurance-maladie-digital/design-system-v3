...
<template>
	<VSheet v-bind="options.sheet" :style="widthStyles" class="vd-sub-header white--text py-6 px-8">
		<slot name="back-btn">
			<VFadeTransition v-if="!hideBackBtn" mode="out-in">
				<div>
					<VThemeProvider>
						<VSkeletonLoader v-if="loading" type="button" class="vd-subheader-loading mb-4" theme="dark" />

						<VBtn v-else v-bind="options.backBtn" class="vd-sub-header-back-btn mb-1"
							:aria-label="effectiveBackBtnAccessibleName" @click="$emit('back')">
							<slot name="back-btn-icon">
								<SyIcon class="mr-2" :icon="backArrowIcon" :decorative="true" />
							</slot>
							{{ backBtnText }}
						</VBtn>
					</VThemeProvider>
		...
		<div class="vd-sub-header-content d-flex justify-space-between flex-wrap ga-8"
			:class="renderFixedHeight ? 'flex-nowrap flex-shrink-0' : 'flex-wrap'">
			<div class="vd-sub-header-informations d-flex flex-column mr-10"
				:class="{ 'flex-shrink-0': renderFixedHeight }">
				<VThemeProvider>
					<slot name="title">
						<VFadeTransition mode="out-in">
							<HeaderLoading v-if="loading" width="300" theme="dark" height="2rem" />
							<SyHeading v-else-if="titleText" class="text-h5 font-weight-bold"
								:aria-label="titleAccessibleName" :level="2">
								{{ titleText }}
							</SyHeading>
						</VFadeTransition>
					</slot>
					...
					<slot name="sub-title">
						<VFadeTransition v-if="subTitleText" mode="out-in">
							<HeaderLoading v-if="loading" class="mt-1" width="250" height="2rem" theme="dark" />
							<p v-else class="text-h6 font-weight-bold mt-1 mb-0"
								:style="{ color: 'rgba(255, 255, 255, .7)' }" :aria-label="subTitleAccessibleName">
								{{ subTitleText }}
							</p>
						</VFadeTransition>
					</slot>
			...
			<slot name="right-content">
				<VThemeProvider theme="dark">
					<DataListGroup v-if="dataListGroupItems" :items="dataListGroupItems" :loading="loading"
						item-width="auto" :class="renderFixedHeight ? 'flex-nowrap flex-shrink-0' : 'flex-wrap'"
						@click:list-item="emitItemAction">
						<template v-if="$slots.item" #item="{ item, index, itemValue }">
							<slot name="item" :item="item" :index="index" :item-value="itemValue" />
						</template>
					</DataListGroup>
				</VThemeProvider>
			</slot>
