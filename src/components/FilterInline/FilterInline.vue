...
<template>
	<div class="sy-filters-inline d-flex flex-wrap max-width-none ma-n2">
		<VMenu v-for="filter in filters" :eager=true :key="filter.name" :close-on-content-click="false" min-width="300px"
			width="400px" offset="12" z-index="1000">
			<template #activator="{ props: menuProps }">
				<VBtn v-bind="menuProps" :class="`sy-filter-${filter.name}`"
					:variant="getFilterCount(filter) ? undefined : 'outlined'" color="secondary" rounded
					class="ma-2 SyFilterBtn">
					<VBadge v-if="getFilterCount(filter)" :content="getFilterCount(filter)"
						:label="locales.badgeLabel(getFilterCount(filter))" inline class="ml-n1 mr-1" />

					{{ filter.title }}

					<SyIcon :icon="mdiChevronDown" :decorative="true" size="small" class="ml-1" />
				</VBtn>
			</template>

			<VSheet color="white" class="pa-4">
				<ChipList :items="getChips(filter)" :overflow-limit="filter.chipOverflowLimit" class="mb-5" 
					:vuetify-options="{ chip: { color: 'secondary' }, icon: { color: 'white' } }"
					@remove="removeChip(filter, $event)" @reset="resetFilter(filter)"/>
				<slot :props="{
