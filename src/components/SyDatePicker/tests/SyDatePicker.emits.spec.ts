...
describe('SyDatePicker - event emissions (no duplicates)', () => {
	afterEach(() => {
		vi.useRealTimers()
...
describe('SyDatePicker - public events', () => {
	afterEach(() => {
		vi.useRealTimers()
		document.body.innerHTML = ''
	})
	...
	it('should emit update:view when the calendar view changes', async () => {
		...
		const yearBtn = wrapper.findComponent({ name: 'SyDatePickerHeader' }).find('.visual-picker-year-btn')
	...
	it('should emit select:year and select:month when a year then a month are selected in the visual picker', async () => {
		...
		const monthBtn = wrapper.findComponent({ name: 'SyDatePickerHeader' }).find('.visual-picker-month-btn')
