...
describe('SyDatePicker props integration', () => {
	afterEach(() => {
		vi.useRealTimers()
	...
	it('forwards root attributes and listeners to the text field only', async () => {
		...
		expect(wrapper.find('.sy-date-picker').attributes('data-tracking-id')).toBeUndefined()
	...
	it('uses overridden locales in the input, popup controls and required validation', async () => {
		...
		expect(document.body.querySelector('.sy-date-picker-header__nav--prev')?.getAttribute('aria-label')).toBe('Previous month')
