...
describe('SyDatePicker - Navigation integration', () => {
	it('navigates to January 2026 via the year and month selectors', () => {
		...
		cy.mountWithVuetify(SyDatePicker, {
			props: {
		...
		cy.get('.sy-date-picker-input__toggle-btn').click()
		cy.get('.sy-date-picker-menu').should('be.visible')
		...
		cy.matchImageSnapshot('sy-date-picker-january-2026', cy.get('.sy-date-picker-menu'))
	})
})
