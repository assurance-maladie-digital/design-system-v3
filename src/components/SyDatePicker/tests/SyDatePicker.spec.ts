...
import { SyDatePicker } from '@/components/index'
import { calendarLocalesKey } from '@/components/Common/Calendar/locales'
import SyDatePickerComponent from '../SyDatePicker.vue'
import SyDatePickerHeader from '../SyDatePickerVisual/SyDatePickerHeader.vue'
...
describe('SyDatePicker', () => {
	afterEach(() => {
		vi.useRealTimers()
		document.body.innerHTML = ''
	})

	it('should expose SyDatePicker as the canonical public export', () => {
		expect(SyDatePicker).toBeDefined()
		expect(SyDatePicker).toBe(SyDatePickerComponent)
	})

	it('should render SyDatePicker', () => {
		const wrapper = mount(SyDatePickerComponent, {
			props: {
		...
		expect(wrapper.find('.sy-date-picker').exists()).toBeTruthy()
	...
	it('should render month and year labels from the displayed calendar month in the header', () => {
		const wrapper = mount(SyDatePickerHeader, {
			props: {
	...
	it('should disable previous/next month navigation when the view is not the calendar days view', () => {
		...
		expect(wrapper.find('.sy-date-picker-header__nav--prev').attributes('disabled')).toBeDefined()
		expect(wrapper.find('.sy-date-picker-header__nav--next').attributes('disabled')).toBeDefined()
		wrapper.unmount()
	})
	...
	it('switches the month button label to the unselected variant when navigating away from the selection', async () => {
		...
		const nextMonthBtn = document.body.querySelector('.sy-date-picker-header__nav--next') as HTMLElement
		nextMonthBtn.click()
		await nextTick()
	...
	describe('SyDatePickerVisual', () => {
		it('should render a dedicated month navigation header in the date picker', async () => {
			...
			await openMenu(wrapper)
			const menuEl = document.body.querySelector('.sy-date-picker-menu') as HTMLElement
			expect(menuEl.querySelector('.sy-date-picker-header__nav--prev')).toBeTruthy()
			expect(menuEl.querySelector('.sy-date-picker-header__nav--next')).toBeTruthy()
			expect(menuEl.querySelector('.sy-date-picker-header__label')?.textContent).toContain(new Intl.DateTimeFormat(navigator.language, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(2026, 8, 4)))

			await (menuEl.querySelector('.sy-date-picker-header__nav--next') as HTMLElement).click()
		...
		it('keeps the input text in sync when the parent updates the single-date value through v-model', async () => {
			...
			const wrapper = mount({
				components: { SyDatePicker: SyDatePickerComponent },
				template: '<SyDatePicker v-model="selectedDate" label="Début du projet" />',
				setup() {
					return { selectedDate }
				},
			}, {
		...
		it('should update the header label only when modelValue changes', async () => {
			...
			const headerDate = wrapper.findComponent(SyDatePickerHeader).find('.visual-picker-header__date')
			...
			const monthPillButton = wrapper.findComponent(SyDatePickerHeader).find('.visual-picker-month-btn')
		...
		it('should reset the visual calendar when the modelValue is cleared', async () => {
			...
			const monthButtonHeader = wrapper.findComponent(SyDatePickerHeader).find('.visual-picker-month-btn')
		...
		it('should switch to the months view when the header pill is clicked, then back to days when a month is selected', async () => {
			...
			expect(wrapper.findComponent({ name: 'MonthSelector' }).isVisible()).toBeTruthy()
			expect(wrapper.findComponent(SyDatePickerHeader).find('.visual-picker-header__date').text()).toBe(new Intl.DateTimeFormat(navigator.language, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(2026, 8, 4)))
		...
		it('should switch to the years view from the days view, then back to days when a year is selected', async () => {
			...
			const yearPillButton = wrapper.findComponent(SyDatePickerHeader).find('.visual-picker-year-btn')
		...
		it('should not open the menu when disabled', async () => {
			...
			expect(document.body.querySelector('.sy-date-picker-menu')).toBeFalsy()
