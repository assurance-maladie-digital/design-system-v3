...
describe('SyDatePicker multiple v-model integration', () => {
	afterEach(() => {
		vi.useRealTimers()
	...
	it('keeps the parent dates and text input synchronized after typing, visual selection and an external update', async () => {
		...
		const wrapper = mount({
			...
			template: '<SyDatePicker v-model="selectedDates" mode="multiple" separator=", " label="Dates" />',
		}, {
		...
		const picker = wrapper.findComponent({ name: 'SyDatePicker' })
