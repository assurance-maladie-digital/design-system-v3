...
const meta: Meta<typeof SyDatePicker> = {
	title: 'Composants/Formulaires/SyDatePicker',
	...
	fieldRequired?: (label?: string) => string,
		...
		'input': {
			...
			table: {
				type: {
					summary: 'SyDatePickerInputSlotProps',
					detail: `{
	inputProps: SyDatePickerInputProps, // props du champ + état de validation, à v-bind sur le champ personnalisé
	...
	toggleBtnRef: Ref<HTMLButtonElement | null>, // à attacher via :ref au bouton d'ouverture du sélecteur visuel
		...
		'menu': {
			...
			table: {
				type: {
					summary: 'SyDatePickerMenuSlotProps',
					detail: `{
	...
	setOpen: (value: boolean) => void, // ex. setOpen(false) pour fermer après sélection
		...
		'header': {
			...
			table: {
				type: {
					summary: 'SyDatePickerHeaderSlotProps',
					detail: `{
	view: 'days' | 'months' | 'years',
	...
	isPreviewEnd: boolean,
		...
		} as Meta<typeof SyDatePicker>['argTypes']),
		'prepend': {
