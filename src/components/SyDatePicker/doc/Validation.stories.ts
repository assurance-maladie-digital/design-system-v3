...
type SyDatePickerStoryProps = SyDatePickerProps & {
...
const meta: Meta<SyDatePickerStoryProps> = {
	title: 'Composants/Formulaires/SyDatePicker/Validation',
	...
	parameters: {
		...
		docs: {
			description: {
				component: 'Exemples de validation pour le composant SyDatePicker.',
			},
		},
	},
	argTypes: {
...
export const WithError: Story = {
	...
	parameters: {
		sourceCode: [{
			name: 'Template',
			code: `<SyDatePicker v-model="selectedDate" label="Date de début" :error-messages="['La date de début ne peut pas être postérieure à la date de fin.']" />`,
		}, {
			name: 'Script',
			code: `import { SyDatePicker } from '@cnamts/synapse'\nimport { ref } from 'vue'\n\nconst selectedDate = ref<Date | undefined>()`,
		}],
	},
}
...
export const WithWarning: Story = {
	...
	parameters: {
		sourceCode: [{
			name: 'Template',
			code: `<SyDatePicker v-model="selectedDate" label="Date de début" :warning-messages="['Cette date est inhabituelle, veuillez la vérifier.']" />`,
		}, {
			name: 'Script',
			code: `import { SyDatePicker } from '@cnamts/synapse'\nimport { ref } from 'vue'\n\nconst selectedDate = ref<Date | undefined>()`,
		}],
...
export const WithSuccess: Story = {
	...
	parameters: {
		sourceCode: [{
			name: 'Template',
			code: `<SyDatePicker v-model="selectedDate" label="Date de début" show-success-messages :success-messages="['La date est valide.']" />`,
		}, {
			name: 'Script',
			code: `import { SyDatePicker } from '@cnamts/synapse'\nimport { ref } from 'vue'\n\nconst selectedDate = ref<Date | undefined>()`,
		}],
...
export const DisableErrorHandling: Story = {
	parameters: {
		...
		sourceCode: [{
			name: 'Template',
			code: `<SyDatePicker v-model="selectedDate" label="Avec validation interne" required />\n<SyDatePicker v-model="selectedDate" label="Validation interne désactivée" required disable-error-handling />`,
		}, {
			name: 'Script',
			code: `import { SyDatePicker } from '@cnamts/synapse'\nimport { ref } from 'vue'\n\nconst selectedDate = ref<Date | undefined>()`,
		}],
	...
	render: args => ({
		...
		template: `
			<div class="d-flex flex-column" style="gap: 16px;">
				<SyDatePicker v-bind="args" :model-value="value" label="Avec validation interne" required @update:model-value="updateValue" />
				<SyDatePicker v-bind="args" :model-value="value" label="Validation interne désactivée" required disable-error-handling @update:model-value="updateValue" />
			</div>
		`,
	}),
}
...
export const CustomRules: Story = {
	parameters: {
		...
		sourceCode: [{
			name: 'Template',
			code: `<SyDatePicker v-model="selectedDate" label="Date de début" :custom-rules="customRules" />`,
		}, {
			name: 'Script',
...
export const NoValidateOnBlur: Story = {
	...
	parameters: {
		...
		sourceCode: [{
			name: 'Template',
			code: `<SyDatePicker v-model="selectedDate" label="Date de début" required :is-validate-on-blur="false" />`,
		}, {
			name: 'Script',
			code: `import { SyDatePicker } from '@cnamts/synapse'\nimport { ref } from 'vue'\n\nconst selectedDate = ref<Date | undefined>()`,
		}],
...
export const SyFormValidation: Story = {
	parameters: {
		sourceCode: [{
			name: 'Template',
			code: `<SyForm @submit="onSubmit">\n\t<SyDatePicker v-model="selectedDate" label="Date de début" required display-asterisk />\n\t<VBtn type="submit">Soumettre</VBtn>\n\t<p v-if="result">{{ result }}</p>\n</SyForm>`,
		}, {
			name: 'Script',
			code: `import { SyDatePicker, SyForm } from '@cnamts/synapse'\nimport { ref } from 'vue'\n\nconst selectedDate = ref<Date | undefined>()\nconst result = ref('')\nconst onSubmit = ({ isValid }: { isValid: boolean }) => {\n\tresult.value = isValid ? 'Formulaire valide.' : 'Formulaire invalide.'\n}`,
		}],
	},
	render: args => ({
		components: { SyDatePicker, SyForm, VBtn },
		setup() {
			const value = ref<Date | undefined>()
			const result = ref('')
			const updateValue = (nextValue: Date | undefined) => {
		...
		template: `
			<SyForm @submit="onSubmit">
				<SyDatePicker v-bind="args" :model-value="value" label="Date de début" required display-asterisk @update:model-value="updateValue" />
...
export const VFormValidation: Story = {
	parameters: {
		sourceCode: [{
			name: 'Template',
			code: `<VForm @submit.prevent="onSubmit">\n\t<SyDatePicker ref="datePicker" v-model="selectedDate" label="Date de début" required display-asterisk />\n\t<VBtn type="submit">Soumettre</VBtn>\n\t<p v-if="result">{{ result }}</p>\n</VForm>`,
		}, {
			name: 'Script',
	...
	render: args => ({
		components: { SyDatePicker, VBtn, VForm },
		setup() {
			const value = ref<Date | undefined>()
			const datePicker = ref<{ validateOnSubmit: () => Promise<boolean> } | null>(null)
			const result = ref('')
		...
		template: `
			<VForm @submit.prevent="onSubmit">
				<SyDatePicker ref="datePicker" v-bind="args" :model-value="value" label="Date de début" required display-asterisk @update:model-value="updateValue" />
				<VBtn class="mt-4" color="primary" type="submit">Soumettre</VBtn>
				<p v-if="result" class="mt-2">{{ result }}</p>
			</VForm>
		`,
...
export const SyFormVuetifyValidation: Story = {
	parameters: {
		sourceCode: [{
			name: 'Template',
			code: `<SyForm @submit="onSubmit">\n\t<SyDatePicker v-model="selectedDate" label="Date de début" />\n\t<VBtn type="submit">Soumettre</VBtn>\n\t<p v-if="result">{{ result }}</p>\n</SyForm>`,
		}, {
			name: 'Script',
			code: `import { SyDatePicker, SyForm } from '@cnamts/synapse'\nimport { ref } from 'vue'\n\nconst selectedDate = ref<Date | undefined>()\nconst result = ref('')\nconst onSubmit = ({ isValid }: { isValid: boolean }) => {\n\tresult.value = isValid ? 'Formulaire valide.' : 'Formulaire invalide.'\n}`,
		}],
	...
	render: args => ({
		...
		template: `
			<SyForm @submit="onSubmit">
				<SyDatePicker v-bind="args" :model-value="value" label="Date de début" @update:model-value="updateValue" />
				<VBtn class="mt-4" color="primary" type="submit">Soumettre</VBtn>
				<p v-if="result" class="mt-2">{{ result }}</p>
			</SyForm>
		`,
