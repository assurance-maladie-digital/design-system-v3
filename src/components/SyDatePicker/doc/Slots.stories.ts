...
const meta: Meta<typeof SyDatePicker> = {
	title: 'Composants/Formulaires/SyDatePicker/Slots',
	component: SyDatePicker,
	parameters: {
		docs: {
			description: {
				component: 'Exemples de personnalisation des slots de SyDatePicker. Les slots permettent d’enrichir le champ de saisie, de modifier le menu de sélection et de surcharger le rendu visuel du calendrier sans remplacer tout le comportement du composant.',
			},
		},
	},
}
		...
		sourceCode: [
			{
				...
				code: `
				<template>
					<SyDatePicker v-model="selectedDate" label="Date de rendez-vous" help-text="Choisissez une date de consultation" clearable>
						<template #prepend>
		...
		sourceCode: [
			{
				...
				code: `
				<template>
					<SyDatePicker v-model="selectedDate" label="Début de mission" help-text="Choisissez la date de début">
						<template #prepend-inner>
...
export const TriggerOnLeft: Story = {
	...
	render: args => ({
		components: { SyDatePicker, SyDatePickerInput, SyIcon },
		setup() {
			const { value } = useBoundValue(args.modelValue as Date | undefined)
			watch(
				() => args.modelValue,
	...
	parameters: {
		docs: {
			description: {
				story: 'Le slot `input` reste un wrapper autour du champ par défaut. On réutilise `SyDatePickerInput` et on branche un bouton personnalisé à gauche via `toggleBtnRef`, sans remplacer toute la logique de validation, de parse et d’ouverture du sélecteur.',
			},
		},
		sourceCode: [
			{
				...
				code: `
				<template>
					<SyDatePicker v-model="selectedDate" label="Date de rendez-vous">
						<template #input="{ inputProps, textValue, updateTextValue, setFocused, toggleBtnRef }">
							<SyDatePickerInput
								:mode="mode"
								:text-value="textValue"
		...
		sourceCode: [
			{
				...
				code: `
				<template>
					<SyDatePicker v-model="selectedDate" label="Date de rendez-vous" clearable>
						<template #header="{ previousMonth, nextMonth, currentMonth }">
		...
		sourceCode: [
			{
				...
				code: `
				<template>
					<SyDatePicker v-model="selectedDate" label="Date de début" clearable>
						<template #menu="{ view, modelValue, setOpen }">
