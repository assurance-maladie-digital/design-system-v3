...
export interface UseSyDatePickerSelectionParams {
	modelValue: Ref<SyDatePickerValue>
	mode: Ref<SyDatePickerMode>
}

export function useSyDatePickerSelection({
	modelValue,
	mode,
}: UseSyDatePickerSelectionParams) {
	/**
	 ...
	 */
	const selectedDateRange = computed<SyDatePickerRange | undefined>(() => {
		const value = modelValue.value
