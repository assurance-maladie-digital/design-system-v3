...
import type { SyDatePickerMode, SyDatePickerMultiple, SyDatePickerRange, SyDatePickerValue } from '../types'
...
function isDateRange(value: unknown): value is SyDatePickerRange {
	return Array.isArray(value)
		&& value.length === 2
		&& value.every(isDateValue)
}

function isDateMultiple(value: unknown): value is SyDatePickerMultiple {
...
export function formatValue(mode: SyDatePickerMode, value: SyDatePickerValue, inputFormat: string, separator: string, locale: string): string | undefined {
	if (mode === 'range') {
...
export function parseValue(mode: SyDatePickerMode, value: string | null, inputFormat: string, separator: string, locale: string): SyDatePickerValue {
...
export function sameValue(mode: SyDatePickerMode, a: SyDatePickerValue, b: SyDatePickerValue): boolean {
	if (mode === 'range') {
