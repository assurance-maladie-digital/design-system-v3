...
import { useSyDatePickerTextSync } from '../useSyDatePickerTextSync'

describe('useSyDatePickerTextSync', () => {
	it('parses localized month names from the typed text', async () => {
		const internalValue = ref<SyDatePickerValue | undefined>(undefined)
		const textValue = ref<string | null | undefined>(undefined)
		const emittedValues: SyDatePickerValue[] = []
	...
	it('formats the initial model value into the text field', () => {
		const internalValue = ref<SyDatePickerValue>(new Date(2025, 8, 3))
		const textValue = ref<string | null | undefined>(undefined)
		useSyDatePickerTextSync({
			mode: 'single',
			inputFormat: 'D MMMM YYYY',
